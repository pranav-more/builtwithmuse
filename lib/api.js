  ...
  if (parts[1] === "admin") {
    ...
    if (req.method === "GET" && parts[2] === "overview") {
      ...
      const activity = await pool.query(
        `SELECT * FROM (
           SELECT 'claim' AS kind, cl.created_at, c.code, left(cl.device_id, 8) AS who, NULL::text AS detail FROM claims cl JOIN codes c ON c.id = cl.code_id
           UNION ALL
           SELECT 'report', r.created_at, c.code, left(r.device_id, 8), NULL FROM reports r JOIN codes c ON c.id = r.code_id
           UNION ALL
           SELECT 'code', c.created_at, c.code, c.source, c.invite_link FROM codes c
           UNION ALL
           SELECT 'waitlist', w.created_at, NULL, w.country, w.email FROM waitlist w
    ...
    if (req.method === "GET" && parts[2] === "codes" && parts.length === 3) {
      ...
      params.push(limit);
      const r = await pool.query(
        `SELECT c.id, c.code, c.invite_link, c.source, c.claims, c.max_claims, c.reports, c.retired_at, c.created_at,
    ...
    if (req.method === "GET" && (parts[2] === "claims" || parts[2] === "reports") && parts.length === 3) {
      ...
      const table = parts[2];
      const r = await pool.query(
        `SELECT t.id, t.created_at, c.code, c.retired_at, left(t.device_id, 8) AS device, left(t.ip_hash, 8) AS ip
    ...
    if (req.method === "GET" && parts[2] === "waitlist" && parts.length === 3) {
      ...
      const r = await pool.query("SELECT id, email, country, created_at FROM waitlist ORDER BY created_at DESC LIMIT $1", [limit]);
      return reply(200, { ok: true, rows: r.rows });
    }
