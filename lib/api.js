...
// Owner budget = platform claims so far + his remaining invites in the Muse
// app (the app count is the source of truth). Update whenever he reports a
// new remaining count; never set below the already handed out count.
let staticSitemapCache = null;
function staticSitemap() {
  if (!staticSitemapCache) staticSitemapCache = fs.readFileSync(path.join(__dirname, "..", "public", "sitemap.xml"), "utf8");
  return staticSitemapCache;
}

// Owner budget 71: 52 handed out + 19 remaining per Muse app on 2026-09-25
const OWNER_MAX_CLAIMS = 71;
...
async function handleApi(req, res, url) {
  ...
  if (parts[1] === "v1" || parts[1] === "pages" || (parts[1] === "admin" && parts[2] === "pool")) {
    const handled = await creators.handle(req, res, url, { reply, ip, ua, body: () => readJson(req), staticSitemap });
