...
.pic-row { display: flex; gap: 14px; align-items: flex-start; margin: 4px 0 8px; }
.pic-row input[type="file"] { font-size: 14px; margin-bottom: 8px; }
.quiet-link { background: none; border: 0; padding: 0; color: var(--muted); font-size: 13px; cursor: pointer; text-decoration: underline; }
.quiet-link:hover { color: var(--ink); }
.opt-details { border: 1px solid var(--line); border-radius: 8px; padding: 0 16px; background: rgba(255,255,255,.03); }
.opt-details summary { cursor: pointer; padding: 14px 0; font-weight: 700; color: var(--muted); }
.opt-details summary:hover { color: var(--ink); }
.opt-details .opt-body { padding: 4px 0 16px; display: grid; gap: 2px; }
...
const BODY = `
        <section class="hero">
          <div>
            ...
            <p class="lead">Answer three quick questions and it goes live the moment you publish. Ratings from other people build your reputation.</p>
          </div>
        ...
        <div class="dash">
          ...
          <section class="panel hidden" id="profile">
            ...
            <form id="profile-form" class="form-stack" novalidate>
              ...
              <div class="row2">
                ...
                <div class="field" data-field="avatar"><label>Profile picture</label>
                  <div class="pic-row">
                    <div class="avatar" id="p-pic-preview" style="width:56px;height:56px;flex:0 0 auto"></div>
                    <div>
                      <input id="p-pic" type="file" accept="image/*" aria-label="Upload a profile picture" />
                      <input type="hidden" id="p-avatar" name="avatarUrl" />
                      <div><button class="quiet-link" type="button" id="p-pic-remove">Remove picture</button></div>
                    </div>
                  </div>
                  <small>JPG or PNG. It is cropped to a square automatically.</small>
                </div>
              </div>
              <div class="row2">
          ...
          <section class="panel hidden" id="editor">
            ...
            <form id="wf-form" class="form-stack" novalidate>
              ...
              <p class="sub" style="margin:0 0 4px">Three quick answers publish a workflow. The rest is optional detail you can add now or later.</p>
              <div class="field" data-field="title"><label for="w-title">What were you trying to do? <span id="w-title-n">10 to 70 characters</span></label><input id="w-title" name="title" maxlength="70" placeholder="Turn a messy inbox into a daily action list" /></div>
              <div class="field" data-field="prompt"><label for="w-prompt">What did you tell Muse? <span>the exact prompt, readers copy this</span></label><textarea id="w-prompt" name="prompt" maxlength="1000" rows="4" placeholder="Example: Look at my last 200 Gmail messages and turn every actionable item into a checklist grouped by today, this week and later."></textarea></div>
              <div class="field" data-field="result"><label for="w-result">What happened? <span>honest outcome, including limits</span></label><textarea id="w-result" name="result" maxlength="1000" rows="4" placeholder="Example: It found 34 actionable items and grouped them well. It missed two calendar invites, so I still check those by hand."></textarea></div>
              <details class="opt-details">
                <summary>Add more detail (optional, helps it rank)</summary>
                <div class="opt-body">
                  ...
                  <div class="field" data-field="problem"><label for="w-problem">The problem <span>optional, 1000 characters</span></label><textarea id="w-problem" name="problem" maxlength="1000" placeholder="What is the person trying to do, and when is this useful?"></textarea></div>
                  ...
                  <div class="field" data-field="steps"><label>Steps <span>optional, 2 to 15, in order</span></label><div class="steps-edit" id="steps"></div><div><button class="btn" type="button" id="add-step">Add a step</button></div></div>
                  <div class="field" data-field="screenshot"><label>Screenshot <span>optional proof</span></label>
                    <div class="pic-row">
                      <img id="w-shot-preview" alt="" style="display:none;max-width:220px;border:1px solid var(--line);border-radius:8px" />
                      <div>
                        <input id="w-shot" type="file" accept="image/*" aria-label="Upload a screenshot" />
                        <input type="hidden" id="w-screenshot" name="screenshot_url" />
                        <div><button class="quiet-link" type="button" id="w-shot-remove">Remove screenshot</button></div>
                      </div>
                    </div>
                    <small>Crop out names, faces and anything personal before uploading. A screenshot supports your result, it never replaces your three answers.</small>
                  </div>
                </div>
              </details>
              <div class="actions">
...
const SCRIPT = `
      (() => {
        ...
        const MESSAGES = {
          ...
          display_name_short: "Display name is too short.", display_name_long: "Display name is too long.", bio_long: "Bio is too long.", website_invalid: "Website must start with http.", avatar_invalid: "Picture must be a JPG or PNG, or an https image link.",
          ...
          prompt_long: "Prompt is over 1000 characters.", proof_url_invalid: "Proof link must start with http.", screenshot_invalid: "Screenshot must be a JPG or PNG under the size limit.", category_required: "Pick a category.", tag_invalid: "Tags must be 2 to 24 characters of letters, numbers and hyphens.",
        ...
        $("edit-profile").addEventListener("click", () => {
          const c = me.creator;
          $("p-handle").value = c.handle; $("p-name").value = c.displayName; $("p-bio").value = c.bio || ""; $("p-web").value = c.websiteUrl || ""; setPic(c.avatarUrl || "", c.displayName);
        ...
        function setPic(url, name) {
          $("p-avatar").value = url || "";
          $("p-pic-preview").innerHTML = url ? '<img src="' + esc(url) + '" alt="" referrerpolicy="no-referrer" />' : esc(String(name || "?").split(/\s+/).map((p) => p[0]).join("").slice(0, 2).toUpperCase());
        }
        $("p-pic").addEventListener("change", () => {
          const file = $("p-pic").files[0]; if (!file) return;
          if (!file.type.startsWith("image/")) { setStatus("profile-status", "Pick an image file.", "err"); return; }
          const img = new Image();
          img.onload = () => {
            ...
            setPic(canvas.toDataURL("image/jpeg", 0.85), me.creator.displayName);
            URL.revokeObjectURL(img.src);
          };
          img.onerror = () => setStatus("profile-status", "Could not read that image.", "err");
          img.src = URL.createObjectURL(file);
        });
        $("p-pic-remove").addEventListener("click", () => { setPic("", me.creator.displayName); $("p-pic").value = ""; });
        // ----- workflow screenshot (optional proof) -----
        function setShot(url) {
          $("w-screenshot").value = url || "";
          const img = $("w-shot-preview");
          if (url) { img.src = url; img.style.display = "block"; } else { img.removeAttribute("src"); img.style.display = "none"; }
        }
        $("w-shot").addEventListener("change", () => {
          const file = $("w-shot").files[0]; if (!file) return;
          if (!file.type.startsWith("image/")) { setStatus("editor-status", "Pick an image file.", "err"); return; }
          const img = new Image();
          img.onload = () => {
          ...
          img.onerror = () => setStatus("editor-status", "Could not read that image.", "err");
          img.src = URL.createObjectURL(file);
        });
        $("w-shot-remove").addEventListener("click", () => { setShot(""); $("w-shot").value = ""; });
        ...
        function fillEditor(w) {
          ...
          f.prerequisites.value = w ? (w.prerequisites || []).join("\\n") : ""; f.prompt.value = w ? w.prompt || "" : ""; f.result.value = w ? w.result || "" : ""; f.proof_url.value = w ? w.proof_url || "" : ""; setShot(w ? w.screenshot_url || "" : ""); f.tags.value = w ? (w.tags || []).join(", ") : "";
        ...
        function readEditor() {
          const f = $("wf-form");
          return { title: f.title.value, summary: f.summary.value, category: f.category.value, problem: f.problem.value, prerequisites: f.prerequisites.value.split("\\n").map((s) => s.trim()).filter(Boolean), steps: [...stepsEl.querySelectorAll("textarea")].map((t) => t.value.trim()).filter(Boolean), prompt: f.prompt.value, result: f.result.value, proof_url: f.proof_url.value, screenshot_url: f.screenshot_url.value, tags: f.tags.value };
        }
