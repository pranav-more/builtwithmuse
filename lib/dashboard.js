...
const SCRIPT = `
      (() => {
        ...
        const MESSAGES = {
          ...
          prompt_long: "Prompt is over 1000 characters.", proof_url_invalid: "Proof link must start with http.", category_required: "Pick a category.",
        ...
        $("profile-form").addEventListener("submit", async (e) => {
          ...
          try {
            await api("/api/v1/me/profile", { method: "PATCH", body: { handle: f.handle.value, displayName: f.displayName.value, bio: f.bio.value, websiteUrl: f.websiteUrl.value, avatarUrl: f.avatarUrl.value } });
        ...
        function fillEditor(w) {
          ...
          f.prerequisites.value = w ? (w.prerequisites || []).join("\\n") : ""; f.prompt.value = w ? w.prompt || "" : ""; f.result.value = w ? w.result || "" : ""; f.proof_url.value = w ? w.proof_url || "" : "";
        ...
        function readEditor() {
          const f = $("wf-form");
          return { title: f.title.value, summary: f.summary.value, category: f.category.value, problem: f.problem.value, prerequisites: f.prerequisites.value.split("\\n").map((s) => s.trim()).filter(Boolean), steps: [...stepsEl.querySelectorAll("textarea")].map((t) => t.value.trim()).filter(Boolean), prompt: f.prompt.value, result: f.result.value, proof_url: f.proof_url.value };
        }
