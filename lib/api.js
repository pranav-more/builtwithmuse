...
async function initSchema(attempts = 30) {
  ...
  for (let i = 1; i <= attempts; i++) {
    try {
      ...
      await pool.query(
        "UPDATE codes SET owner_creator_id = (SELECT id FROM creators WHERE lower(email) = lower($2) LIMIT 1) WHERE upper(code) = upper($1) AND source = 'owner' AND owner_creator_id IS NULL",
