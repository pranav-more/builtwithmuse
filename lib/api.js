  ...
  if (req.method === "GET" && url.pathname === "/api/pool") {
    const r = await pool.query(
      `SELECT left(code, 1) AS first, least(length(code) - 1, 5) AS dots, claims, max_claims, reports, redeemed,
         ...
         FROM codes
        ORDER BY (retired_at IS NULL AND claims < max_claims) DESC, (retired_at IS NULL) DESC, created_at DESC
        LIMIT 12`
    );
    const totals = (await pool.query(
      `SELECT count(*) AS codes, coalesce(sum(max_claims - claims), 0) AS open
         FROM codes WHERE retired_at IS NULL AND claims < max_claims`
    )).rows[0];
    return reply(200, {
      ok: true,
