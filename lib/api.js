...
//   POST /api/claim     hand out an invite code. Google sign in is required;
//                       the owner's own codes are handed out first, always,
//                       regardless of report counts. Fresh unverified codes
//                       get the first pick, then codes with verified balances
//                       and recent confirmations, then everything else.
//                       The pick and the claim row update happen in one
//                       statement with FOR UPDATE SKIP LOCKED, so concurrent
//                       visitors never receive the same code. An account keeps
//                       the code it was given: a plain claim returns the same
//                       code again, and a new one is handed out only with
//                       { fresh: true }, which the page sends after the holder
//                       flags the code as not working.
//   POST /api/report    flag a code that did not work; two flags pull it from
//                       the pool (never the owner's code: a spike there means
//                       he should check his Muse app instead).
...
//   POST /api/codes     add a code to the pool (signed in; needs the full
//                       invite link and the current invite count from the
//                       owner's Muse app).
//   GET  /api/v1/me/codes  the signed in account's own codes, to confirm the
//                       invite count or remove a code.
...
// and a Google signed in account are required for a claim; claims are counted
// per account, per device, and per hashed IP in the database (hour and day
// windows); obvious non-browser user agents are refused; the forms carry a
// honeypot field; codes are only revealed on a click, never in the HTML.
//
...
// These are seeded into the pool on boot with priority 1 so his code is
// handed out before anyone else's. Standing rule (2026-09-26): his code
// stays up until he asks for it to be removed. The budget renews itself
// automatically (see OWNER_RENEW_CHUNK and claimCode); he no longer needs to
// report his Muse app invite balance for top-ups. Append new codes he sends.
const OWNER_CODES = ["70Z09F"];
// Initial claim budget for the owner's code. The budget now renews itself
// whenever it runs out, so this number only matters on first seeding and
// is never allowed to shrink on redeploy (see the boot upsert below).

// Owner budget 87: 71 handed out + 16 remaining per Muse app on 2026-09-26
const OWNER_MAX_CLAIMS = 87;
// Seats automatically added whenever the owner's code runs out. His code is
// meant to stay up until he asks for it to be removed.
const OWNER_RENEW_CHUNK = 100;
...
CREATE UNIQUE INDEX IF NOT EXISTS blog_ratings_once_idx ON blog_ratings (slug, device_id);
...
-- One account owns one code: ownership, owner reported balance, and when
-- they last confirmed it. Claims, confirmed redemptions and reports are
-- keyed to the account so one person cannot consume the same code twice.
ALTER TABLE codes ADD COLUMN IF NOT EXISTS owner_creator_id UUID REFERENCES creator_profiles(id) ON DELETE SET NULL;
ALTER TABLE codes ADD COLUMN IF NOT EXISTS owner_invites INT;
ALTER TABLE codes ADD COLUMN IF NOT EXISTS last_verified_at TIMESTAMPTZ;
ALTER TABLE claims ADD COLUMN IF NOT EXISTS creator_id UUID REFERENCES creator_profiles(id) ON DELETE CASCADE;
CREATE UNIQUE INDEX IF NOT EXISTS claims_code_creator_uidx ON claims (code_id, creator_id) WHERE creator_id IS NOT NULL;
ALTER TABLE redemptions ADD COLUMN IF NOT EXISTS creator_id UUID REFERENCES creator_profiles(id) ON DELETE CASCADE;
CREATE UNIQUE INDEX IF NOT EXISTS redemptions_code_creator_uidx ON redemptions (code_id, creator_id) WHERE creator_id IS NOT NULL;
ALTER TABLE reports ADD COLUMN IF NOT EXISTS creator_id UUID REFERENCES creator_profiles(id) ON DELETE CASCADE;
CREATE UNIQUE INDEX IF NOT EXISTS reports_code_creator_uidx ON reports (code_id, creator_id) WHERE creator_id IS NOT NULL;
UPDATE codes SET last_verified_at = coalesce(last_verified_at, created_at) WHERE last_verified_at IS NULL;
-- Account based rows do not always arrive with a device cookie attached.
ALTER TABLE claims ALTER COLUMN device_id DROP NOT NULL;
ALTER TABLE redemptions ALTER COLUMN device_id DROP NOT NULL;
ALTER TABLE reports ALTER COLUMN device_id DROP NOT NULL;
...
async function poolCreator(req) {
  try {
    const c = await creators.creatorFromRequest(req);
...
// Picks the next code for a signed in account. The account's held code comes
// back when it is still alive, otherwise the highest tier fresh code they
// have not already claimed. Tier order: owner code first, then newly added
// codes (48h boost), then recently verified codes, then stale codes last.
// Broken codes never pick. Stale codes sink to the bottom but stay pickable.
async function claimCode(client, creator, deviceId, ip, fresh, where) {
  await client.query("BEGIN");
  try {
    if (!fresh) {
      // The account's current code comes back instead of revealing a new one,
      // unless it was retired or this account flagged it as not working.
      // The owner's code is never pulled by report counts, so it is exempt.
      const recent = await client.query(
        `SELECT c.id, c.code, c.invite_link, c.source,
                EXISTS (SELECT 1 FROM redemptions rd WHERE rd.code_id = c.id AND rd.creator_id = $1) AS redeemed
           FROM claims cl JOIN codes c ON c.id = cl.code_id
          WHERE cl.creator_id = $1 AND c.retired_at IS NULL
            AND (c.source = 'owner' OR c.reports < 2)
            AND NOT EXISTS (SELECT 1 FROM reports r WHERE r.code_id = c.id AND r.creator_id = $1)
      ...
      if (row) {
        ...
        return { code: { id: row.id, code: row.code, invite_link: row.invite_link, source: row.source }, reused: true, redeemed: Boolean(row.redeemed) };
      }
    }
    // The pick: owner codes first, always, no matter the report count or how
    // long since the last confirmation. A code added in the last 24 hours
    // with exactly 2 invites left jumps the queue so its scarce invites get
    // used fast. Codes down to their last reported invite sink below
    // everything else. Stale codes sink to the bottom of the order but are
    // never excluded; only retired or twice reported codes stay out of the
    // pool.
    const picked = await client.query(
      `SELECT c.id, c.code, c.invite_link, c.source FROM codes c
        WHERE c.retired_at IS NULL
          AND (c.source = 'owner' OR c.reports < 2)
          AND NOT EXISTS (SELECT 1 FROM claims cl WHERE cl.code_id = c.id AND cl.creator_id = $1)
        ORDER BY
          CASE WHEN c.source = 'owner' THEN 0 ELSE 1 END,
          CASE WHEN c.source = 'user' AND c.created_at > now() - interval '24 hours' AND c.owner_invites = 2 THEN 0 ELSE 1 END,
          CASE WHEN c.owner_invites IS NOT NULL AND c.owner_invites <= 1 THEN 1 ELSE 0 END,
          CASE WHEN c.source = 'user' AND c.created_at > now() - interval '48 hours' THEN 0 ELSE 1 END,
          CASE WHEN c.last_verified_at > now() - interval '7 days' THEN 0 ELSE 1 END,
    ...
    await client.query(
      "INSERT INTO claims (code_id, creator_id, device_id, ip_hash, country, region, city) VALUES ($1, $2, $3, $4, $5, $6, $7) ON CONFLICT DO NOTHING",
      [row.id, creator.id, deviceId || null, ip, where.country, where.region, where.city]
    );
    await client.query("COMMIT");
    return { code: { id: row.id, code: row.code, invite_link: row.invite_link, source: row.source }, reused: false, redeemed: false };
  } catch (err) {
...
async function handleApi(req, res, url) {
  ...
  // Creator pool: /api/v1/*, the rendered pages under /api/pages/*, and the
  // moderation endpoints under /api/admin/pool/*.
  if (parts[1] === "v1" || parts[1] === "pages" || (parts[1] === "admin" && parts[2] === "pool")) {
    const handled = await creators.handle(req, res, url, { reply, ip, ua, body: () => readJson(req) });
  ...
  if (req.method === "GET" && url.pathname === "/api/stats") {
    const r = await pool.query(
      `SELECT
         (SELECT count(*) FROM codes WHERE retired_at IS NULL) AS live,
         (SELECT count(*) FROM codes) AS submitted,
         (SELECT count(*) FROM waitlist) AS waitlist`
    );
    const row = r.rows[0];
    return reply(200, { ok: true, live: Number(row.live), submitted: Number(row.submitted), waitlist: Number(row.waitlist) });
  }

  // Blog post ratings: thumbs up / thumbs down, one vote per device per post.
  if (req.method === "GET" && url.pathname === "/api/blog/ratings") {
    const slug = (url.searchParams.get("slug") || "").toLowerCase().replace(/[^a-z0-9-]/g, "").slice(0, 120);
    if (!slug) return reply(400, { ok: false, error: "missing slug" });
    const counts = await pool.query(
      `SELECT count(*) FILTER (WHERE vote = 1) AS up, count(*) FILTER (WHERE vote = -1) AS down
         FROM blog_ratings WHERE slug = $1`,
      [slug]
    );
    let userVote = 0;
    const device = deviceFromCookie(req);
    if (device) {
      const mine = await pool.query("SELECT vote FROM blog_ratings WHERE slug = $1 AND device_id = $2", [slug, device]);
      if (mine.rows[0]) userVote = mine.rows[0].vote;
    }
    return reply(200, { ok: true, up: Number(counts.rows[0].up), down: Number(counts.rows[0].down), userVote });
  }

  if (req.method === "POST" && url.pathname === "/api/blog/rate") {
    if (!ua || BOT_UA.test(ua)) return reply(403, { ok: false, error: "forbidden" });
    const body = await readJson(req);
    const slug = String(body.slug || "").toLowerCase().replace(/[^a-z0-9-]/g, "").slice(0, 120);
    const vote = body.vote === -1 ? -1 : body.vote === 1 ? 1 : 0;
    if (!slug || !vote) return reply(400, { ok: false, error: "bad request" });
    let device = deviceFromCookie(req);
    const extra = {};
    if (!device) {
      const header = deviceCookieHeader(req);
      device = header.split("=")[1].split(".")[0];
      extra["Set-Cookie"] = header;
    }
    await pool.query(
      `INSERT INTO blog_ratings (slug, device_id, vote) VALUES ($1, $2, $3)
       ON CONFLICT (slug, device_id) DO UPDATE SET vote = EXCLUDED.vote`,
      [slug, device, vote]
    );
    const counts = await pool.query(
      `SELECT count(*) FILTER (WHERE vote = 1) AS up, count(*) FILTER (WHERE vote = -1) AS down
    ...
    return reply(200, { ok: true, up: Number(counts.rows[0].up), down: Number(counts.rows[0].down), userVote: vote }, extra);
  }

  // Public view of the pool: masked codes and seat counts only, never a code.
  // Public view of the pool: masked codes and honest counts only, never a code.
  if (req.method === "GET" && url.pathname === "/api/pool") {
    const viewer = await poolCreator(req);
    const r = await pool.query(
      `SELECT c.id, left(c.code, 1) AS first, least(length(c.code) - 1, 5) AS dots,
              c.reports, c.redeemed, c.source, c.owner_invites, c.last_verified_at, c.created_at,
              (SELECT count(*) FROM claims cl WHERE cl.code_id = c.id) AS claimed_by,
              CASE WHEN $1::uuid IS NOT NULL THEN EXISTS (SELECT 1 FROM claims cl WHERE cl.code_id = c.id AND cl.creator_id = $1) ELSE false END AS mine
         FROM codes c
        WHERE c.retired_at IS NULL
        ORDER BY
          CASE WHEN c.source = 'owner' THEN 0 ELSE 1 END,
    ...
    const totals = (await pool.query(
      `SELECT count(*) AS codes,
              ...
              count(*) FILTER (WHERE reports >= 2 AND source <> 'owner') AS broken
         FROM codes WHERE retired_at IS NULL`
    )).rows[0];
    const claimed = await pool.query("SELECT count(*) AS n FROM claims");
    return reply(200, {
      ok: true,
      codes: Number(totals.codes),
      claimed: Number(claimed.rows[0].n),
      verifiedCodes: Number(totals.verified),
      brokenCodes: Number(totals.broken),
      rows: r.rows.map((row) => {
        const verifiedDays = row.last_verified_at ? Math.floor((Date.now() - new Date(row.last_verified_at).getTime()) / 86400000) : 0;
        // The owner's code is never pulled by report counts or confirmation age.
        const state = row.source !== "owner" && row.reports >= 2 ? "pulled" : verifiedDays >= 7 ? "stale" : "open";
        return {
          id: row.id,
          label: row.first.toUpperCase() + "\u2022".repeat(Math.max(1, Number(row.dots))),
          claimedBy: Number(row.claimed_by || 0),
          verified: Number(row.redeemed || 0) > 0,
          isNew: row.source === "user" && Date.now() - new Date(row.created_at).getTime() < 48 * 3600 * 1000,
          state,
          mine: Boolean(row.mine),
          ownerInvites: row.owner_invites,
          checkedDays: verifiedDays,
        };
      }),
    }, { "Cache-Control": "private, max-age=20" });
  }

  // Only the signed in account that holds a code may confirm it worked.
  if (req.method === "POST" && url.pathname === "/api/redeemed") {
    if (!ua || BOT_UA.test(ua)) return reply(403, { ok: false, error: "forbidden" });
    const creator = await poolCreator(req);
    ...
    const found = await pool.query(
      `SELECT c.id FROM codes c WHERE upper(c.code) = upper($1) AND c.retired_at IS NULL
          AND EXISTS (SELECT 1 FROM claims cl WHERE cl.code_id = c.id AND cl.creator_id = $2)`,
      [code, creator.id]
    );
    if (!found.rows[0]) return reply(404, { ok: false, error: "unknown_code" });
    const inserted = await pool.query(
      "INSERT INTO redemptions (code_id, creator_id, device_id) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING RETURNING id",
      [found.rows[0].id, creator.id, deviceFromCookie(req)]
    );
    if (inserted.rows[0]) await pool.query("UPDATE codes SET redeemed = redeemed + 1 WHERE id = $1", [found.rows[0].id]);
    return reply(200, { ok: true });
  }

  // Claim a code for a signed in account. A reveal is a claim: it shows the
  // code, it does not spend one of the owner's real invites.
  if (req.method === "POST" && url.pathname === "/api/claim") {
    if (!ua || BOT_UA.test(ua)) return reply(403, { ok: false, error: "forbidden" });
    const creator = await poolCreator(req);
    if (!creator) return reply(401, { ok: false, error: "sign_in_required" });
    const body = await readJson(req);
    if (await overLimit(pool, "claims", "creator_id", creator.id, LIMITS.claim.device)) return reply(429, { ok: false, error: "rate_limited" });
    if (await overLimit(pool, "claims", "ip_hash", ip, LIMITS.claim.ip)) return reply(429, { ok: false, error: "rate_limited" });
    const client = await pool.connect();
    try {
      const result = await claimCode(client, creator, deviceFromCookie(req), ip, Boolean(body.fresh), geo(req));
      if (!result) return reply(404, { ok: false, error: "no_codes" });
      return reply(200, { ok: true, code: result.code.code, inviteLink: result.code.invite_link, source: result.code.source, reused: result.reused, redeemed: result.redeemed });
    } finally {
      client.release();
    }
  }

  // One report per account per code. Two reports pull a community code;
  // owner codes never auto retire.
  if (req.method === "POST" && url.pathname === "/api/report") {
    ...
    const found = await pool.query(
      `SELECT c.id, c.reports, c.source FROM codes c
        WHERE upper(c.code) = upper($1) AND c.retired_at IS NULL
          AND EXISTS (SELECT 1 FROM claims cl WHERE cl.code_id = c.id AND cl.creator_id = $2)`,
      [code, creator.id]
    );
    if (!found.rows[0]) return reply(404, { ok: false, error: "unknown_code" });
    ...
    const where = geo(req);
    const inserted = await pool.query(
      "INSERT INTO reports (code_id, creator_id, device_id, ip_hash, country, region, city) VALUES ($1, $2, $3, $4, $5, $6, $7) ON CONFLICT DO NOTHING RETURNING id",
      [codeId, creator.id, deviceFromCookie(req), ip, where.country, where.region, where.city]
    );
    if (inserted.rows[0]) {
      // Two flags pull a code from the pool. The owner's code is exempt: a
      // spike there means he should check his Muse app, not lose the code.
      await pool.query("UPDATE codes SET reports = reports + 1 WHERE id = $1", [codeId]);
    }
    return reply(200, { ok: true });
  }

  // Submit a code: the signed in account owns it and reports how many invites
  // it really has. Adding a code that already exists links the existing row
  ...
  if (req.method === "POST" && url.pathname === "/api/codes") {
    ...
    const code = String(body.code || "").trim();
    // The Muse app shares the link inside a longer message. Pull the URL out
    // of whatever was pasted instead of rejecting a genuine share.
    const rawLink = String(body.inviteLink || body.invite_link || "").trim();
    const linkMatch = rawLink.match(/https?:\/\/[^\s<>"']+/i);
    const link = linkMatch ? linkMatch[0].replace(/[.,;:!?)\]]+$/, "") : "";
    const rawInvites = body.ownerInvites !== undefined ? body.ownerInvites : body.invites;
    const invites = rawInvites === undefined || rawInvites === null || rawInvites === "" ? null : Math.floor(Number(rawInvites));
    if (!CODE_RE.test(code)) return reply(400, { ok: false, error: "bad_code" });
    if (!link || link.length > 500) return reply(400, { ok: false, error: "bad_link" });
    // The invite count is required: it is what the pool shows and ranks on.
    if (invites === null || !Number.isFinite(invites) || invites < 1 || invites > 100000) return reply(400, { ok: false, error: "invites_required" });
    if (await submissionLimitedByKind("submit", ip)) return reply(429, { ok: false, error: "rate_limited" });
    const where = geo(req);
    const existing = await pool.query("SELECT id, owner_creator_id FROM codes WHERE upper(code) = upper($1)", [code]);
    if (existing.rows[0]) {
      const e = existing.rows[0];
      if (e.owner_creator_id && e.owner_creator_id !== creator.id) return reply(409, { ok: false, error: "already_owned" });
      // Resubmitting your own code updates the count and clears old flags: you
      // checked and it works now. The first signed in account to re add a
      // legacy unowned code adopts it.
      await pool.query(
        "UPDATE codes SET owner_creator_id = $2, owner_invites = $3, last_verified_at = now(), invite_link = $4, reports = 0, retired_at = NULL WHERE id = $1",
        [e.id, creator.id, invites, link]
      );
      return reply(200, { ok: true, added: true, adopted: !e.owner_creator_id });
    }
    await pool.query(
      "INSERT INTO codes (code, invite_link, source, owner_creator_id, owner_invites, last_verified_at, country, region, city) VALUES ($1, $2, 'user', $3, $4, now(), $5, $6, $7)",
      [code.toUpperCase(), link, creator.id, invites, where.country, where.region, where.city]
    );
    return reply(200, { ok: true, added: true });
  }

  // The owner confirms how many invites they really have left.
  if (req.method === "POST" && url.pathname === "/api/codes/verify") {
    const creator = await poolCreator(req);
    if (!creator) return reply(401, { ok: false, error: "sign_in_required" });
    const body = await readJson(req);
    ...
    const rawInvites = body.ownerInvites !== undefined ? body.ownerInvites : body.invites;
    const invites = rawInvites === undefined || rawInvites === null || rawInvites === "" ? null : Math.floor(Number(rawInvites));
    if (!Number.isFinite(id) || invites === null || !Number.isFinite(invites) || invites < 1 || invites > 100000) {
      return reply(400, { ok: false, error: "bad_request" });
    }
    const r = await pool.query(
      "UPDATE codes SET owner_invites = $3, last_verified_at = now() WHERE id = $1 AND owner_creator_id = $2 AND retired_at IS NULL RETURNING id",
      [id, creator.id, invites]
    );
    if (!r.rows[0]) return reply(404, { ok: false, error: "unknown_code" });
    return reply(200, { ok: true });
  }

  // The owner pulls their own code from the pool. The seeded owner code is
  // protected: Jayesh's code can only be removed when he says so himself.
  if (req.method === "POST" && url.pathname === "/api/codes/retire") {
    const creator = await poolCreator(req);
    if (!creator) return reply(401, { ok: false, error: "sign_in_required" });
    const body = await readJson(req);
    const id = Number(body.id);
    if (!Number.isFinite(id)) return reply(400, { ok: false, error: "bad_request" });
    const r = await pool.query(
      "UPDATE codes SET retired_at = now() WHERE id = $1 AND owner_creator_id = $2 AND source <> 'owner' AND retired_at IS NULL RETURNING id",
      [id, creator.id]
    );
    if (!r.rows[0]) return reply(404, { ok: false, error: "unknown_code" });
    return reply(200, { ok: true });
  ...
  if (parts[1] === "admin") {
    ...
    if (req.method === "GET" && parts[2] === "overview") {
      const r = await pool.query(
        `SELECT
           (SELECT count(*) FROM codes WHERE retired_at IS NULL AND (source = 'owner' OR reports < 2)) AS live,
           (SELECT count(*) FROM codes WHERE retired_at IS NULL AND reports >= 2 AND source <> 'owner') AS broken,
           ...
           (SELECT count(DISTINCT creator_id) FROM claims WHERE created_at > now() - interval '1 day') AS claimers_24h`
      );
    ...
    if (req.method === "GET" && parts[2] === "analytics") {
      ...
      const daily = await pool.query(
        `WITH d AS (SELECT generate_series((now() at time zone 'utc')::date - ($1::int - 1), (now() at time zone 'utc')::date, '1 day')::date AS day)
         SELECT d.day,
           ...
           (SELECT count(DISTINCT creator_id) FROM claims WHERE (created_at at time zone 'utc')::date = d.day) AS claimers,
      ...
      const pool_ = (await pool.query(
        `SELECT
           ...
           (SELECT count(DISTINCT creator_id) FROM claims WHERE creator_id IS NOT NULL) AS claimers_total,
           (SELECT count(*) FROM claims) AS claims_total,
           (SELECT count(*) FROM reports) AS reports_total`
      )).rows[0];
    ...
    if (req.method === "GET" && parts[2] === "codes" && parts.length === 3) {
      ...
      const where = [];
      const params = [];
      if (status === "live") where.push("c.retired_at IS NULL AND (c.source = 'owner' OR c.reports < 2)");
      if (status === "boosted") where.push("c.retired_at IS NULL AND c.source = 'user' AND c.created_at > now() - interval '24 hours' AND c.owner_invites = 2");
      if (status === "broken") where.push("c.retired_at IS NULL AND c.reports >= 2 AND c.source <> 'owner'");
      if (status === "retired") where.push("c.retired_at IS NOT NULL");
      if (q) { params.push("%" + q + "%"); where.push(`upper(c.code) LIKE $${params.length}`); }
      params.push(limit);
      const r = await pool.query(
        `SELECT c.id, c.code, c.invite_link, c.source, c.claims, c.reports, c.redeemed, c.retired_at, c.created_at, c.country, c.region, c.city,
                c.owner_invites, c.last_verified_at,
                (c.source = 'user' AND c.created_at > now() - interval '24 hours' AND c.owner_invites = 2) AS boosted,
                (SELECT count(*) FROM claims cl WHERE cl.code_id = c.id) AS claims_live,
    ...
    if (req.method === "POST" && parts[2] === "codes" && parts[3] === "retire") {
      ...
      const code = String(body.code || "").trim();
      if (!CODE_RE.test(code)) return reply(400, { ok: false, error: "bad_code" });
      // The seeded owner code is protected: Jayesh's code stays until he says so.
      const r = await pool.query("UPDATE codes SET retired_at = coalesce(retired_at, now()) WHERE upper(code) = upper($1) AND source <> 'owner' RETURNING id", [code]);
      return reply(200, { ok: true, retired: Boolean(r.rows[0]) });
    }

    if (req.method === "DELETE" && (parts[2] === "codes" || parts[2] === "waitlist") && /^\d+$/.test(parts[3] || "")) {
      // The seeded owner code cannot be deleted from the admin panel.
      const guard = parts[2] === "codes" ? "AND source <> 'owner'" : "";
      const r = await pool.query(`DELETE FROM ${parts[2]} WHERE id = $1 ${guard} RETURNING id`, [Number(parts[3])]);
...
async function initSchema(attempts = 30) {
  ...
  for (let i = 1; i <= attempts; i++) {
    try {
      ...
      await pool.query("INSERT INTO codes (code, source, priority, max_claims) SELECT upper(c), 'owner', 1, $2 FROM unnest($1::text[]) AS t(c) ON CONFLICT (upper(code)) DO UPDATE SET source='owner', priority=1, max_claims = GREATEST(codes.max_claims, $2)", [OWNER_CODES, OWNER_MAX_CLAIMS]);
      // Link the seeded owner code to Jayesh's creator profile so it shows in
      // his My Codes and he can confirm the invite count himself. The seed
      // upsert above never touches owner_creator_id, so this sticks. The
      // retire endpoint still refuses source='owner' rows, so the code cannot
      // be removed by accident.
      await pool.query(
        "UPDATE codes SET owner_creator_id = (SELECT id FROM creator_profiles WHERE lower(email) = lower($2) LIMIT 1) WHERE upper(code) = upper($1) AND source = 'owner' AND owner_creator_id IS NULL",
        [OWNER_CODES[0], "jayeshmarathe2000jm@gmail.com"]
      );
...
creators.init({
  pool, DEVICE_SECRET, GOOGLE_CLIENT_ID, BOT_UA,
  parseCookies, deviceFromCookie, isSecure, verifyGoogleIdToken, adminIdentity, isAdmin,
});

// Pretty paths served by the function (Vercel rewrites them here, server.js
// forwards them directly): the workflow index, workflow and creator pages
// and the sitemap with dynamic entries appended.
const PAGE_ROUTES = [
  [/^\/workflows\/?$/, () => "pages/workflows"],
  [/^\/creators\/?$/, () => "pages/creators"],
  [/^\/creator\/?$/, () => "pages/creator"],
  [/^\/workflows\/([^/]+)\/?$/, (m) => "pages/workflows/" + m[1]],
  [/^\/creators\/([^/]+)\/?$/, (m) => "pages/creators/" + m[1]],
  [/^\/sitemap-pool\.xml$/, () => "pages/sitemap"],
];
function pageRoute(pathname) {
  for (const [re, to] of PAGE_ROUTES) {
...
async function handle(req, res) {
  ...
  if (carried !== null) {
    ...
    const routed = pageRoute(url.pathname);
    if (routed) url.pathname = "/api/" + routed;
  }
...
module.exports = { handle, ensureReady, pageRoute };
