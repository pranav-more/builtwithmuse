...
async function claimCode(client, deviceId, ip, fresh, where) {
  ...
  try {
    // Owner codes never run dry: if his code is fully used, top up the budget
    // automatically so it keeps being handed out first. He removes the code
    // (via admin) when his Muse app is actually out of invites.
    await client.query(
      "UPDATE codes SET max_claims = claims + $1 WHERE source = 'owner' AND retired_at IS NULL AND claims >= max_claims",
      [OWNER_RENEW_CHUNK]
    );

  if (!fresh) {      // The device's current code comes back instead of burning a new one, unless
