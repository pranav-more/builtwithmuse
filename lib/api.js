...
ALTER TABLE codes ADD COLUMN IF NOT EXISTS country TEXT, ADD COLUMN IF NOT EXISTS region TEXT, ADD COLUMN IF NOT EXISTS city TEXT;
...
async function handleApi(req, res, url) {
  ...
  if (parts[1] === "admin") {
    ...
    if (req.method === "GET" && parts[2] === "codes" && parts.length === 3) {
      ...
      params.push(limit);
      const r = await pool.query(
        `SELECT c.id, c.code, c.invite_link, c.source, c.claims, c.max_claims, c.reports, c.retired_at, c.created_at, c.country, c.region, c.city,
